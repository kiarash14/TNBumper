...
local function create_group(msg)
    ...
    if not is_admin(msg) then
        return "This service is private for SUDO (@kiarash_gh14)"
    end
...
local function lock_group_name(msg, data)
    ...
    local group_name_lock = data[tostring(msg.to.id)]['settings']['l_n']
	if group_name_lock == 'yes' then
	    return 'Name is already locked'
	else
	    data[tostring(msg.to.id)]['settings']['l_n'] = 'yes'
	    save_data(_config.moderation.data, data)
	    data[tostring(msg.to.id)]['settings']['s_n'] = string.gsub(msg.to.print_name, '_', ' ')
	    save_data(_config.moderation.data, data)
...
local function unlock_group_name(msg, data)
	...
	if group_name_lock == 'no' then
	    ...
	    data[tostring(msg.to.id)]['settings']['l_n'] = 'no'
	    save_data(_config.moderation.data, data)
...
local function lock_group_member(msg, data)
    ...
    local group_member_lock = data[tostring(msg.to.id)]['settings']['l_m']
	if group_member_lock == 'yes' then
	    return 'Members are already locked'
	else
	    data[tostring(msg.to.id)]['settings']['l_m'] = 'yes'
	    save_data(_config.moderation.data, data)
	end
...
local function unlock_group_member(msg, data)
	...
	if group_member_lock == 'no' then
	    ...
	    data[tostring(msg.to.id)]['settings']['l_m'] = 'no'
	    save_data(_config.moderation.data, data)
...
local function lock_group_photo(msg, data)
    ...
    local group_photo_lock = data[tostring(msg.to.id)]['settings']['l_ph']
	if group_photo_lock == 'yes' then
	    return 'Photo is already locked'
	else
	    data[tostring(msg.to.id)]['settings']['s_ph'] = 'waiting'
	    save_data(_config.moderation.data, data)
	end
...
local function unlock_group_photo(msg, data)
	...
	if group_photo_lock == 'no' then
	    ...
	    data[tostring(msg.to.id)]['settings']['l_ph'] = 'no'
	    save_data(_config.moderation.data, data)
...
function run(msg, matches)
    ...
    if data[tostring(msg.to.id)] then
		local settings = data[tostring(msg.to.id)]['settings']
		if matches[1] == 'sa' and matches[2] then
		    deskripsi = matches[2]
